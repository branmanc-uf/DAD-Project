# Insider Trading Detection Simulation (300+ lines) with Adaptive Enforcement and Removal
import numpy as np
import pandas as pd
...
# === STEP 2: Encode Trader Preferences ===
...
def run_iterative_simulation(rounds, num_insiders, num_normals, num_noise, initial_flag_k):
    ...
    for r in range(1, rounds + 1):
        ...
        threshold = max(0.9, scores['P_insider'].quantile(0.98 - 0.03 * r)) if not scores.empty else 0.9
        results, metrics = evaluate_detection(traders, scores, threshold)

        flag_top_k = max(10, initial_flag_k - r)
        if metrics['recall'] < 0.2:
            flag_top_k = min(50, flag_top_k + 5)
        ...
        for i, row in traders.iterrows():
            ...
            if row['status'] == 'suspended':
                ...
                if traders.at[i, 'suspension_count'] >= 2:
                    traders.at[i, 'status'] = 'removed'
...
if __name__ == "__main__":
    metrics_df, tracking_df, final_traders = run_iterative_simulation(
        rounds=24, num_insiders=100, num_normals=100, num_noise=100, initial_flag_k=100)
...
def plot_pct_growth_by_risk(tracking_df):
    ...
    # === Option 4: Percentage Growth Over Time by Risk ===
